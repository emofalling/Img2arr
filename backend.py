...
"""
from numpy import ndarray, array, zeros, empty, uint8, intc
            ...
            for extname in os.listdir(ctype_fullpath):
                ...
                if EXT_OP_CDLL in reload_feautures:
                    ...
                    if not os.path.isfile(path):
                        ...
                        continue
                    try:
                        ext = _load_exts_cdll(path, regname_prefix)
                    except Exception as e:
                        errf(path, e)
                        continue
...
def _load_exts_cdll(file: str, regname_prefix: str) -> CDLL:
    """load_exts子函数：加载CDLL"""
    # 先加载
...
class Pre_iter:
    ...
    def next(self, name: str, args: c_void_p, argsize: int, is_head: bool = False, is_tail: bool = False) -> tuple[PIPENodeResult, NDArray[uint8]]:
        ...
        *：有一个特殊的虚扩展""(空字符串)，它固有属性为ATTR_REUSE，且只负责将输入复制到输出（如果数组指针不同）。利用它能快速的实现扩展的禁用。
        ...
        # 调用预处理
        if name != "": # 默认逻辑
            ret = self._callpre(name, args, in_buf, out_buf)
        else: # 空扩展，直接复制
    ...
    def _callpre(self, name: str, args: c_void_p, in_buf: MidBuffer, out_buf: MidBuffer | None):
        """调用预处理"""
        # 获取指针
